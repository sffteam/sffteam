...
if(strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') == 0){
	...
	if(strcasecmp($contentType, 'application/json') == 0){
		...
		$hash=strtolower(hash('sha512', $data->key.'|'.$data->txnid.'|'.$data->amount.'|'.$data->pinfo.'|'.$data->fname.'|'.$data->email.'|||||'.$data->udf5.'||||||'.$data->salt));
		$json=array();
		$json['success'] = $hash;
...
function getCallbackUrl()
{
	...
	return $protocol . $_SERVER['HTTP_HOST'] . '/success/';
}
...
<div class="main">
    ...
    <span><input type="text" id="key" name="key" placeholder="Merchant Key" value="kujzlEyB" /></span>
    </div>
    
    <div class="dv">
    <span class="text"><label>Merchant Salt:</label></span>
    <span><input type="text" id="salt" name="salt" placeholder="Merchant Salt" value="HPGXoAsCMq" /></span>
    </div>

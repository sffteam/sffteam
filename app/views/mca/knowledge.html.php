<div class="navbar">
  <div class="navbar-bg bg-color-black"></div>
  <div class="navbar-inner sliding">
    <a href="/mca/" class="external link text-color-white">Home</a>
    <a href="/mca/reports/" class="external link text-color-white">Reports</a>
    <a href="/mca/pdfs/" class="external link text-color-white">PDFs</a>
    <a href="/mca/products/categories/" class="external link text-color-white">Products</a>
    <a href="/mca/videos/" class="external link text-color-white">Videos</a>
  </div>
</div>

  <div class="block block-strong">
    <p>All the videos you will need for WeCapacitate Knowledge</p>
  </div>
  
  <div class="list menu-list no-padding no-margin ">
    <ul>
      <li class="margin-left">Survive or Thrive - Ruchi Doctor for Tamilnadu team Palaniappan D
        <a
          href="#" class="no-margin no-padding">
          <div class="item-inner">
            
             <iframe width="100%" src="https://www.youtube.com/embed/ewsHkUznp6c" class="lazy"></iframe>
            
          </div>
        </a>
      </li>
      
            
      
      <li class="text-align-center">End of List</li>
      <li>&nbsp;</li>
      <li>&nbsp;</li>
    </ul>
  </div>

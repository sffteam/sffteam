...
class DistributorsController extends \lithium\action\Controller {
...
//      return $this->redirect('/dashboard');
 ...
 public function information() {
   Savings::meta('connection', 'Distributor');			
			$detail = Savings::count();
			print_r($detail."\n");
			
			Savings::meta('connection', 'default');			
			$detail = Savings::count();
			print_r($detail."\n");
   return $this->render(array('json' => array("success"=>"Yes")));		      
 }
...
//    'ancestors'=>$user['ancestors'],
    ...
    'summary'=>array(
     $yyyymm=>array(
      'gbv'=>$user['summary'][$yyyymm]['gbv']?:0,
      'pbv'=>$user['summary'][$yyyymm]['pbv']?:0,
      'dp'=>$user['summary'][$yyyymm]['dp']?:0,
     )
    ),
    $yyyymm=>
     array(
 ...
 foreach($users as $u){
  ...
  array_push(
   ...
   array(
    ...
    'summary'=>array(
     $yyyymm=>array(
      'gbv'=>$u['summary'][$yyyymm]['gbv']?:0,
      'pbv'=>$u['summary'][$yyyymm]['pbv']?:0,
      'dp'=>$u['summary'][$yyyymm]['dp']?:0,
     )
    ),
    $yyyymm=>
     array(

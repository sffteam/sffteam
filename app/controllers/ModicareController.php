  ...
  $allparams = array();
    foreach($products as $p){
      $dataParam = array(
       ...
       'caption'=> '<div class="stepper stepper-fill stepper-round stepper-small stepper-init color-red"><div class="stepper-button-minus no-ripple" onclick="minustoCart(\''.$p['Code'].'\');"></div><div class="stepper-input-wrap"><input type="text" style="" value="getValue('.get$p['Code'].')" min="0" max="10" step="1" readonly name="minusCode'.$p['Code'].'" id="minusCode'.$p['Code'].'"></div><div class="stepper-button-plus no-ripple"  onclick="addtoCart(\''.$p['Code'].'\');"></div></div><span class="text-color-yellow"> &nbsp;'.$val.'&nbsp; </span><br>'.$p['Name'].' <br>['.$p['Code'].'] <span class="text-color-red">MRP: <strike>'.number_format($p['MRP'],2).'</strike></span> <span class="text-color-green">DP: '.number_format($p['DP'],2).'<span class="text-color-yellow"> PV: '.number_format($p['PV'],2).'</span> <span class="text-color-green">BV: '.number_format($p['BV'],2).'</span> <span class="text-color-blue">'.number_format($p['BV']/$p['DP']*100,0).'%</span>',
      );

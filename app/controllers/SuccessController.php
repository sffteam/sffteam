...
$CalcHashString = null;
  ...
  if (isset($postdata ['key'])) {
   ...
   if ($status == 'success' && $resphash == $CalcHashString) {
     ...
     $function->addnotify($mcaNumber,"Payment received Rs. ".$postdata['amount'],"We have received your payment through PayUMoney transaction ID: ". $postdata['txnid']);

     if($postdata['amount']=="1110.00"){
      $function = new Functions();
      $function->addnotify($mcaNumber,"Select plan and make a new payment","You have to select a plan and make a next payment");
     }
    ...
    Savings::update($data,$conditions);   
      ...
      $function = new Functions();
      $function->addnotify($mcaNumber,"Payment not complete Rs. ".$postdata['amount'],"Your payment is canceled/not authorized transaction ID: ". $postdata['txnid']);
    
     if($postdata['amount']=="1110.00"){

      $function = new Functions();
      $function->addnotify($mcaNumber,"Repayment","Please retry the payment or call administrator/upline.");
...
//    return $this->redirect('/dashboard/');
 ...
 private function sendEmailTo($email,$mcaNumber){
     ...
     $agreement = Savings::find('first',array(
      'conditions'=>array(
      'mcaNumber'=>$mcaNumber
      )
     ));
